...
function handleUserEntry(entry, sessionData, parserState, pendingTaskIds, taskIdMap, timestamp) {
  ...
  // Human input
    ...
    if (typeof content === 'string') {
      // Detect /compact command start (emit compaction_starting before skipping)
      if (content.includes('<command-name>')) {
        if (content.includes('/compact') || content.includes('compact')) {
          return [{ type: 'compaction_starting', content: 'Compacting context...', timestamp }];
        }
        return null;
      }
