...
function createWatcher(deps) {
  ...
  async function watchSession(sessionId) {
    ...
    // Terminal panes (no log file) — register for command injection only, no file watching
    if (!session.logFile) {
      activeSessions.set(sessionId, {
        ...
        lastStatus: 'terminal',
        contextPercentage: 0,
        compactionStartSent: false,
        subagentWatchers: new Map(),
        subagentPositions: new Map(),
        subagentPollIntervals: new Map(),
