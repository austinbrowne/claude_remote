    ...
    const contextPollInterval = setInterval(async () => {
      try {
        ...
        if (sd && clamped !== sd.contextPercentage) {
          ...
          if (clamped >= 95 && !sd.compactionStartSent) {
          ...
          // Reset flag when context drops significantly (compaction completed)
          if (clamped < 80 && sd.compactionStartSent) {
    ...
    const processFileContent = async (filePath) => {
      ...
      try {
        ...
        if (pendingPermissions.size > 0) {
          ...
          for (const [, deferredItem] of permissionsToApprove) {
            console.log(`[Subagent ${agentId}] Broadcasting permission to clients: ${deferredItem.tool ?? 'unknown'}`);
            broadcastToClients({
              type: 'claude_output',
              sessionId,
              data: deferredItem
