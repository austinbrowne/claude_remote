 ...
 * Used for terminal panes that have no JSONL log — gives the user
 * a snapshot of what's on screen after injecting a command.
 */
async function capturePane(paneId, lines = 50) {
  const targetErr = validateTarget(paneId);
  if (targetErr) throw targetErr;
  const { stdout } = await execFileAsync('tmux', [
