...
async function getActiveProcesses() {
  try {
    const { stdout } = await execFileAsync('tmux', [
      'list-panes', '-a', '-F',
      '#{session_name}|#{pane_id}|#{pane_pid}|#{pane_current_command}|#{pane_tty}|#{pane_current_path}'
    ], { timeout: TMUX_EXEC_TIMEOUT_MS });
    ...
    for (const line of stdout.trim().split('\n')) {
      ...
      const paneTty = parts[4]; // e.g. /dev/ttys010
      const currentPath = parts.slice(5).join('|'); // Rejoin in case path contains '|'
      ...
      results.push({
        ...
        deviceTty: paneTty || null, // e.g. /dev/ttys010 — used by hybrid adapter for dedup
      });
    }
